  ...
  GDash::Dashboard.toplevel name do |zuul_site|
    zuul_site.dashboard :"#{name}_zuul_top" do |dashboard|
      dashboard.dashboard :"#{site[:prefix]}_zuul_compare" do |zuul_compare|
        zuul_compare.title = "Host Comparison"

        zuul_compare.section :title => "Key Metric Comparison", :width => 5 do |compare|
          site[:hosts].each do |host|
            compare.ganglia_report :"#{host}_load_report", :title => "Load" do |ganglia_report|
            ...
            compare.ganglia_graph :"#{host}_request_rate_99th", :title => "Request Rate Total 99 Percentile" do |graph|
              graph.hosts = host
            ...
            compare.ganglia_graph :"#{host}_jvm_heap_usage", :title => "JVM Memory Heap Usage" do |graph|
              graph.hosts = host
            ...
            compare.ganglia_graph :"#{host}_cpu_idle", :title => "CPU Idle Percentile" do |graph|
              graph.hosts = host
